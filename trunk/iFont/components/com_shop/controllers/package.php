 ...
 */
class ShopControllerPackage extends JControllerForm
{
	/**
	 * @since	1.6
	 */
	...
	function ajaxRenderSample() {
		$packageIdsStr = JRequest::getString("packageIds");
		if ($packageIdsStr == "") {
			return null;
		}
		...
		$thumbs = array();
		$packageIds = explode(",", $packageIdsStr);
		$packageModel = JModel::getInstance("Package", "ShopModel");
		foreach ($packageIds as $packageId) {
			...
			$font = $packageModel->getLatestPackageFont($packageId);
			if ($font != null) {
				$fontId = $font->id;
