 ...
 * @since		1.6
 */
class ShopControllerFonts extends JControllerAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 * @since	1.6
	 */
	protected $text_prefix = 'COM_SHOP_FONTS';
	 ...
	 * @since	1.6
	 */
	public function getModel($name = 'Font', $prefix = 'ShopModel', $config = array('ignore_request' => true))
	{
	...
	public function changeBlock()
	{
		...
		if (empty($ids)) {
			JError::raiseWarning(500, JText::_('COM_SHOP_FONTS_NO_ITEM_SELECTED'));
		} else {
			// Get the model.
			$model = $this->getModel();
			...
			if (!$model->block($ids, $value)) {
				...
				if ($value == 1){
					$this->setMessage(JText::plural('COM_SHOP_N_FONTS_BLOCKED', count($ids)));
				} else if ($value == 0){
					$this->setMessage(JText::plural('COM_SHOP_N_FONTS_UNBLOCKED', count($ids)));
				}
			}
		}
